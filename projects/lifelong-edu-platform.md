...
자동 레이블링을 위해 고객사는 Active Learning이라는 기법을 활용할 수 있는지를 질의하였고, 활용 가능하다는 판단하에 Active Learning을 기반으로 자동 레이블링 방안을 설계하였습니다. Active Learning은 "좋은 데이터를 활용한다"면, 적은 labeled 데이터로도 충분한 성능을 내는 모델을 만들 수 있다는 가정을 담은 학습 기법입니다. 주로 이번 주제와 같이 레이블 데이터가 많지 않아 모델 학습 이전에 레이블링 작업이 필요한 경우, 레이블링을 위한 수작업을 최소화하는 방안으로 쓰이며 '무엇이 좋은 데이터인가?'를 결정하는 __쿼리 전략__을 잘 짜는 것이 중요한 요소입니다.

__활용 데이터 및 전처리__

데이터 현황 파악 결과, 강좌명과 강좌 내용이 존재하며 다수 강좌가 연계되는 KOCW, K-MOOC, 한국방송통신대학교 등의 기관 강의를 활용하도록 하였습니다. 강좌명 및 강좌 내용을 임베딩 벡터화하고, 이를 input 데이터로 하는 분류 모델을 설계하였습니다.

__분석 방안 설계__

자동 레이블링 모델 설계를 위해 필요한 고민들은 다음과 같았습니다.

- Active Learning의 쿼리 전략은 무엇을 활용할 것인가?
