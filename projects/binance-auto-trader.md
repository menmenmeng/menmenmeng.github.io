...
> 기본적 매매 전략으로 실제 수익을 벌어들일 수 있는지 확인한다.(백테스트)
> 수익이 날 만한 전략(손해가 크지 않은 안정적 전략)을 찾아내고 웹소켓을 사용한 실시간 트레이더를 구성한다.
> 마지막으로 데이터 분석을 통해, 상승/하락의 패턴이 있는지 머신러닝/딥러닝을 활용해 확인해본다.
> 현재의 volatility나 price를 바탕으로 매일마다 전략에 적절한 하이퍼파라미터를 업데이트하는 실시간 트레이더를 구성한다.
> + 라이브러리를 더 직관적으로 만들어서, 또는 User Interface를 만드는 라이브러리를 활용해서 프로그래밍을 모르는 사람들이 쉽게 매매를 시도해볼 수 있도록 한다.
