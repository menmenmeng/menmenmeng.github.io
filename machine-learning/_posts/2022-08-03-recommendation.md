...
ref : https://velog.io/@ijune97/RecommendationAlgorithm{:.figcaption}
...
협업 필터링의 두 가지 방식에 대해 더 알아보았다.{:.lead}
...
특정 사용자와 유사한 다른 사용자를 TOP-N(N명의 유사 사용자를 추출한다는 뜻으로 보임)으로 선정하여 그들의 취향에 잘 맞는 아이템을 추천한다. 여기서 TOP-N을 선정할 때는 사용자 간 유사하다는 평가를 어떻게 수치적으로 표현할지를 고민해야 하는데, 그와 관련된 내용이 코사인 유사도이다.
무엇을 보고 유사하다고 표현하는가? 사용자 기반의 알고리즘에서는, 유저 A가 평가한 아이템들의 평점과 유저 B가 평가한 아이템들의 평점이 비슷하다면, A, B를 유사하다고 표현한다. 위의 사용자-아이템 평점 행렬을 다시 가져와 보겠다.
...
User1과 User2는 Item1, Item2에 대한 평점이 각각 (1, 2), 그리고 (1, 1)로 두 아이템을 거의 비슷하게 평가했다. 또한 User2가 Item4에 4라는 높은 평점을 준 걸 볼 수 있다.

위의 행렬을 보고 얻어낸 두 사실을 바탕으로, 우리는 "User1은 User2와 비슷하므로, User2가 좋아했던 Item4를 좋아할 가능성이 높다"는 가설을 세울 수 있다.
