...
두 개의 볼륨 경로가 충돌할 경우(ex. container:/app에 바인드 마운트, container:/app/somepath에 익명 볼륨), 더 긴 경로가 우선적으로 적용된다. 이 규칙이 중요한 게, 전체 경로를 볼륨 설정함으로써 덮어씌워지는 컨테이너 내 파일을 익명 볼륨 등으로 살릴 수 있는 기능이기 때문.
