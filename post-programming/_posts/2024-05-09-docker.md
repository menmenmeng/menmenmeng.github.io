...
Anonymous volume과 마찬가지로 직접 access는 할 수 없다. docker에 의해 관리되는, 로컬 머신 내의 어딘가의 장소일 뿐이다. 그렇지만 익명 볼륨과 다르게 컨테이너가 없어져도 그대로 남아 있고, 컨테이너를 다시 켤 때 재사용 가능하다. Named volume은 익명 볼륨과 달리 "이 volume을 사용하겠다"를 명시할 수 있기 때문이다. 컨테이너 간 데이터를 공유하고 싶을 때도 사용할 수 있다.
...
`-v $(pwd):/app` 처럼, pwd를 사용해서 단축 버전으로 명령어를 쓸 수도 있다. 왜냐면 로컬 마신 디렉터리가 엄청 길 수도 있으니까..
...
익명 볼륨 / named volume은, 만약 로컬 볼륨 안에 뭔가 파일이 있다면 해당 내용이 컨테이너 내 볼륨 폴더를 덮어쓴다. 만약 로컬 볼륨 안에 아무것도 없다면, 컨테이너 내 볼륨의 내용이 로컬 볼륨에 복사된다.

바인드 마운트는, 로컬 볼륨 내용이 컨테이너 볼륨 안으로 덮어씌워진다. 바인드 마운트의 경우 로컬 볼륨 안에 내용이 없더라도, 컨테이너 볼륨을 덮어써 버린다.

두 개의 볼륨 경로가 충돌할 경우(ex. container:/app에 바인드 마운트, container:/app/somepath에 익명 볼륨), 더 긴 경로가 우선적으로 적용된다. 이 규칙이 중요한 게, 전체 경로를 볼륨 설정함으로써 덮어씌워지는 컨테이너 내 파일을 익명 볼륨 등으로 살릴 수가 있기 때문이다.
